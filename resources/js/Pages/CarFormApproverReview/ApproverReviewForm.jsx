...
export default function ReviewForm({auth, forms}){
  ...
  useEffect(() => {
    ...
    if (status === "Approver's%20Review") {
      setSelectedTabIndex(0);
      setSelectedID(id);
    } else if (status === "Approver's%20Revised") {
        ...
        <PageLayout>
          <PageTabGroup selectedIndex={selectedTabIndex} setSelectedIndex={setSelectedTabIndex}>
            <PageTablist>
              <PageTab>
                Review
                <CountForm forms={forms} formStatus={["Approver's Review"]}/>
              </PageTab>
              <PageTab>
                Revised
                <CountForm forms={forms} formStatus={["Approver's Revised"]}/>
              </PageTab>
            </PageTablist>
            <PageTabPanels>
              <PageTabPanel>
                <PageTableLayout forms={forms} formStatus="Approver's Review" title="Review the CAR Forms" roles={auth.roles} selectedID={selectedID}/>
              </PageTabPanel>
              <PageTabPanel>
                <PageTableLayout forms={forms} formStatus="Approver's Revised" title="Review the CAR Forms (Revised)" roles={auth.roles} selectedID={selectedID}/>
              </PageTabPanel>
            </PageTabPanels>
          </PageTabGroup>
        </PageLayout>
