...
export function handleVerificationSubmit(post, closingModal, put, form, data){
  const handleFirstVerificationSubmit = (e) => {
    e.preventDefault();
    Swal.fire({
      title: "Are you sure?",
      ...
      if (result.isConfirmed) {
        post(route('verification_processors.store'), {
          _token: csrfToken,
          onError: (error) => {
            console.log(error);
            Swal.fire({
  ...
  const handleUpdateFirstVerificationSubmit = (e) => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        put(route('verification_processors.update', form.verification[0].id), {
          _token: csrfToken,
          onError: (error) => {
            console.log(error);
            Swal.fire({
